...
# Suppress InsecureRequestWarning BEFORE importing requests
import urllib3
...
def safe_dedupe_list(items: list, max_items: int = 10) -> List[str]:
    """Safely deduplicate a list that may contain dicts or strings.

    Converts dicts to their 'name' or 'id' field, or JSON string as fallback.
    ...
    for item in items:
        if isinstance(item, dict):
            # Try common name fields, fall back to JSON
            value = item.get("name") or item.get("id") or item.get("display_name") or json.dumps(item, sort_keys=True)
        else:
            value = str(item)
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for pulse in pulses[:10]:
                    ...
                    raw_tags.extend(pulse.get("tags", []))
                    raw_malware.extend(pulse.get("malware_families", []))
                    raw_attacks.extend(pulse.get("attack_ids", []))
                ...
                intel = {
                    ...
                    "city": data.get("city", ""),
                    "pulses": pulse_info,
                    # Use safe deduplication for lists that may contain dicts
                    "tags": safe_dedupe_list(raw_tags, 15),
                    "malware_families": safe_dedupe_list(raw_malware, 10),
                    "attack_ids": safe_dedupe_list(raw_attacks, 10)
                }
